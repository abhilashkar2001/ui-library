...
export class NormalLoanEmiCalculatorComponent implements OnInit {
  ...
  loanCalculation(event: any) {
    ...
    this.totalInterest = Number(((this.principlAmount * (this.interestRate * 0.01)) / totalMonths).toFixed(2));
    this.totalAmount = Number((this.principlAmount / totalMonths + this.totalInterest).toFixed(2));
    this.emiAmount = Number((this.principlAmount / totalMonths + this.totalInterest).toFixed(2));
