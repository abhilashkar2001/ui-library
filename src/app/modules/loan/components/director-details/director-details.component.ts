...
export class DirectorDetailsComponent implements OnInit {
  ...
  newCustomer(data?: any) {
    const formGroup = this.fb.group({
      ...
      customerCategory: [data ? data?.customerCategory : ''],
      sharePercentage: [data ? data?.sharePercentage : ''],
      authorisedSignatory: [data ? data?.authorisedSignatory : false],
      source: 'Website',
  ...
  createDocumentGroup(doc: any): FormGroup {
    return this.fb.group({
      ...
      docuemntType: [doc?.docuemntType ?? '', Validators.required],
      kraPin: [doc?.kraPin ?? '', Validators.required],
      tin: [doc?.tin ?? '', Validators.required],
