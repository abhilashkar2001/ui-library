...
export class DesireLimitComponent implements OnInit {
  ...
  initCardControlForm() {
    ...
    this.desiredLimitForm.get('cardNumber').valueChanges.pipe(debounceTime(200)).subscribe(val=>{
      console.log(val);
      if(val){
        this.cardDetailsFetch();
        this.getCreditCardLimitByNo(val);
        this.selecetdCardNo=this.creditCardList.filter(item=>item?.cardNo== val);
