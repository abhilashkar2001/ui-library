...
export class PrepaidRefundComponent implements OnInit {
  refundForm: FormGroup | undefined;
  ...
  fetchCardDetails(event){
    console.log(event)
    this.cardService.fetchRefund(event).subscribe((res:IcHttpResponseModel<any>)=>{
     if(res?.statusCode==200 && res?.data?.length){
        this.cardDetails=[res?.data[0]]
        this.refundForm.get('fullRefund').setValue(res?.data[0]?.fullRefund)
        this.refundForm.get('currencyCode').setValue(res?.data[0]?.currencyCode)
        this.refundForm.get('currencyUnit').setValue(res?.data[0]?.currencyUnit)
        this.refundForm.get('balanceAmount').setValue(res?.data[0]?.balanceAmount)
     }
    })
  }
   ...
   proceed() {
     const selectedCardNumber =
      this.refundForm?.get("fromPrepaidCard")?.value;
