...
@Component({
  selector: "app-dispatch-documents",
  templateUrl: "./dispatch-documents.component.html",
  styleUrls: ["./dispatch-documents.component.scss"],
})
export class DispatchDocumentsComponent implements OnInit {
  ...
  checkform() {
    return this.dispatchForm.valid;
  }
  public get attachMentModel(): FormArray {
    return this.dispatchForm?.get("attachMentModel") as FormArray;
  }
  ...
  addTitleCategory(data?: any) {
    ...
    for (let i = 0; i < this.attachMentModel["controls"]?.length; i++) {
      this.dispatchForm.valueChanges.subscribe((res) => {
