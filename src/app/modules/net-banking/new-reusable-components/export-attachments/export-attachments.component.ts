...
@Component({
  selector: "app-export-attachments",
  templateUrl: "./export-attachments.component.html",
  styleUrls: ["./export-attachments.component.scss"],
})
export class ExportAttachmentsComponent implements OnInit {
  @Input("updateParentModel") updateParentModel: (
    part: Partial<any>,
    isFormValid: boolean
  ) => void;
  exportattachementInfoForm: FormGroup;
  titles: any[] = [
    "BG Text",
    "Contract Copy",
    "Declaration",
  ...
  buildAttachmentInfoForm(data?: any) {
    this.exportattachementInfoForm = this.formBuilder.group({
      attachMentModel: this.formBuilder.array([]),
    });
  }

  checkform() {
    return this.exportattachementInfoForm.valid;
  }
  public get attachMentModel(): FormArray {
    return this.exportattachementInfoForm?.get("attachMentModel") as FormArray;
  }
  ...
  addTitleCategory(data?: any) {
    ...
    for (let i = 0; i < this.attachMentModel["controls"]?.length; i++) {
      this.exportattachementInfoForm.valueChanges.subscribe((res) => {
